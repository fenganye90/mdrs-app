 ...
 * @类描述：
 * 
 * @创建人：Johney @创建时间：2017-12-20 15:42 @version：V1.0
 */
@Controller
@RequestMapping("/auth")
public class AuthController {
	...
	public ModelAndView toPage(Permission permission) {
		...
		try {
			if (null != permission) {
				...
				authServiceImpl.addPermission(permission);
	...
	public ModelAndView permList() {
		...
		try {
			List<Permission> permList = authServiceImpl.permList();
		...
		try {
			if (null != permission) {
				...
				if (0 == type) {
					...
					// 编辑权限
					this.authServiceImpl.updatePerm(permission);
				} else if (1 == type) {
					permission.setCreateTime(date);
					// 增加子节点权限
					this.authServiceImpl.addPermission(permission);
				}
	...
	public Permission getPerm(@RequestParam("id") int id) {
		...
		try {
			if (id > 0) {
				Permission perm = this.authServiceImpl.getPermission(id);
	...
	public String del(@RequestParam("id") int id) {
		...
		try {
			if (id > 0) {
				return this.authServiceImpl.delPermission(id);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Role> getRoleList() {
		...
		try {
			roleList = authServiceImpl.roleList();
	...
	public String addRole(@RequestParam("permIds") String permIds, Role role) {
		...
		try {
			...
			return authServiceImpl.addRole(role, permIds);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public ModelAndView updateRole(@PathVariable("id") Integer id) {
		...
		try {
			...
			RoleVO rvo = this.authServiceImpl.findRoleAndPerms(id);
			// 角色下的权限
			List<RolePermissionKey> rpks = rvo.getRolePerms();
			// 获取全部权限数据
			List<PermissionVO> pvos = authServiceImpl.findPerms();
	...
	public String setRole(@RequestParam("rolePermIds") String permIds, Role role) {
		...
		try {
			...
			return authServiceImpl.updateRole(role, permIds);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String delRole(@RequestParam("id") int id) {
		...
		try {
			if (id > 0) {
				return this.authServiceImpl.delRole(id);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<Role> getRoles() {
		...
		try {
			return this.authServiceImpl.getRoles();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<PermissionVO> getUserPerms() {
		...
		try {
			pvo = authServiceImpl.getUserPerms(existUser.getId());
			// 生成页面需要的json格式
